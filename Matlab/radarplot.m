function f=radarplot(R,Label,LineColor,FillColor,LineStyle,LevelNum)
...
if nargin>1
    if length(Label)>=n
        ...
        for i=1:n
            if ~sum(strcmpi({'' },Label(i)))
                text(LableX(i), LableY(i),cell2mat(Label(i)), 'FontSize',14,'HorizontalAlignment','center','Rotation',0)
            end
        end
    end
else
